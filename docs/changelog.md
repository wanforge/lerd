...
- **Dashboard WorkersWidget and Services page hid store-defined framework workers and lumped them under a generic "Workers" group** (surfaced by #310). Two issues. First, the custom-framework-workers branch in `buildServicesList` resolved the framework via `config.GetFramework`, which returns the built-in Laravel skeleton (queue/schedule/reverb/horizon) and never reads the versioned store YAMLs. Any worker defined in `lerd-frameworks` (the new `vite` worker is the first such case) was therefore invisible to the dashboard's WorkersWidget and the Services page, even though the same worker appeared correctly on the site detail page (which uses `GetFrameworkForDir`). Second, even after that's fixed the frontend's `workerGroups` derived store hardcoded a single catch-all "Workers" bucket for everything not matching queue/schedule/reverb/horizon/stripe, so vite would have appeared under a generic header alongside any other future store-defined worker. The backend now resolves frameworks via `GetFrameworkForDir(s.Framework, s.Path)` and emits `worker_label` alongside `worker_name`; the frontend buckets these by `worker_name` and uses `worker_label` (e.g. "Vite Dev Server") as the group header, so each store-defined worker gets its own labelled section in the sidebar.
